...
| Badge "certificato scaduto" a livello di singola atleta in tono warning (informativo) — **eccetto** in `/conferma-certificati` (Story 9.23), dove è danger per motivi di contesto (vedi Componenti → Badge) | Usare rosso/danger acceso a livello di singola riga fuori da quell'unica eccezione — allarma proprio nel momento in cui si registra la presenza (contro FR-15) |
