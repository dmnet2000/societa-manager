...
components:
  ...
  campo-max-width:
    max-width: '400px'
  menu-profilo:
    trigger-foreground: '{colors.surface-alt}'
    background: '{colors.surface}'
    radius: '{rounded.sm}'
    shadow: '0 1px 3px rgba(16,24,32,0.08)'
    item-foreground: '{colors.text-primary}'
status: final
updated: 2026-07-26
---
...
**Risolto — Story 9.4 (2026-07-26)**: il primo componente transitorio reale del progetto è il menu profilo (dropdown, vedi Componenti sotto), non il wizard Story 6.3 (non ancora costruito). L'ipotesi di lavoro sopra è confermata così com'era: ombra molto leggera (`0 1px 3px rgba(16,24,32,0.08)`, stesso valore hard-coded già riusato ad hoc da `riquadro-form`, Story 9.3) e `{rounded.sm}` (non `{rounded.md}`, riservato a contenitori più ampi). Resta aperta solo per un eventuale futuro dialog di conferma modale, se mai costruito: il menu profilo non è quel caso (nessun `role="dialog"`, vedi Componenti).
...
- **Badge di stato (`badge`)** — pillola arrotondata `{rounded.sm}`, testo `{typography.badge-label}`. Tre varianti semantiche: successo ({colors.success} su {colors.success-bg}), attenzione ({colors.warning} su {colors.warning-bg}) e criticità ({colors.danger} su {colors.danger-bg}). **Regola specifica e non negoziabile**: il badge "Certificato scaduto" mostrato accanto al nome di una singola atleta (es. in una riga presenza) usa la variante **warning**, non danger — coerente con FR-15 ("l'alert non impedisce in nessun caso di registrare la presenza"): a livello di singola persona lo stato deve informare, non allarmare. La variante danger è riservata al conteggio aggregato (vedi stat-tile). **Eccezione esplicita — Story 9.23 (2026-08-02)**: in `/conferma-certificati`, sezione "Confermati", il badge di stato del singolo certificato usa la variante **danger** anche a livello di singola atleta per lo stato "Scaduto" (oltre a successo/attenzione per in regola/in scadenza) — l'unico punto del sistema in cui questa regola non si applica. Motivazione: quella pagina è il contesto di **gestione attiva** dei certificati da parte di Admin/Dirigente/Segreteria (non il flusso presenze che FR-15 protegge dall'essere "allarmante" durante un'azione non correlata) — qui un segnale netto aiuta a dare priorità a un rinnovo urgente. Non estendere questa eccezione ad altre pagine senza una nuova decisione esplicita.

**Seconda eccezione esplicita — Story 9.33, estensione round 4 (2026-08-07)**: la regola sopra riguarda specificamente il badge "Certificato scaduto"/stato certificato. Su `/gruppi`, la tabella Atlete introduce due colonne di stato distinte — Iscrizione e Tesseramento — con badge **successo** (verde, "Iscritta"/"Tesserata") o **danger** (rosso, "Non iscritta"/"Non tesserata") anche a livello di singola atleta. Decisione presa esplicitamente con l'utente dopo un mockup HTML/CSS approvato prima dell'implementazione. Motivazione: come per Story 9.23, `/gruppi` è un contesto di gestione (Admin/Dirigente), non il flusso presenze protetto da FR-15; a differenza del Certificato, Iscrizione/Tesseramento sono stati binari sempre visibili (non un allarme occasionale) — il colore comunica lo stato stesso, non solo un rischio. Non estendere ulteriormente senza una nuova decisione esplicita.
...
- **Riquadro pagina-form (`riquadro-form`)** — pattern riusabile per le pagine il cui contenuto principale è un form (o un piccolo gruppo di form/azioni strettamente correlati sulla stessa entità di configurazione — es. `/smtp` unisce configurazione SMTP e invio email di prova, `/logo` unisce logo e nome del settore) anziché una tabella/elenco (`/registrati`, `/import-atlete`, `/precaricamento-allenatori`, `/permessi-certificati`, `/smtp`, `/logo`): il `<main>` centra un riquadro (`background: {colors.surface}`, bordo `{colors.border}`, `{rounded.md}`, ombra minima `0 1px 3px rgba(16,24,32,0.08)` — nessun token `--shadow` esiste ancora in `DESIGN.md`, riuso ad hoc dello stesso valore hard-coded) invece di stirare i campi a piena larghezza della finestra. `max-width: 480px`, più largo dei 360px del pattern gemello `.pagina`/`.riquadro` di `/accedi` (Story 8.2, modulo CSS locale non toccato da questo pattern globale): questi form hanno più campi visibili contemporaneamente (es. `/smtp`: host+porta+utente+password; `/registrati`: email+password+fieldset ruoli+campi condizionali), un riquadro più stretto avrebbe causato wrapping eccessivo delle label. Su schermo stretto il riquadro occupa `width: 100%` col solo padding di `{spacing.8}`/`{spacing.4}` del contenitore, senza scorrimento orizzontale. Non applicato a `/accedi` (che resta sul proprio pattern locale a 360px) né alle pagine a struttura variabile (`/wizard-nuova-stagione`, `/certificato-medico`) — le pagine-tabella/elenco non usano questo pattern specifico (il riquadro con bordo/ombra), ma da questa estensione (2026-07-26) condividono comunque il vincolo generale di larghezza massima di pagina, vedi voce successiva.
- **Menu profilo (dropdown) (`menu-profilo`)** — Story 9.4 (2026-07-26): risolve la `[NOTA UX APERTA]` sulle superfici sovrapposte transitorie (vedi Elevazione e Profondità sopra), primo componente reale di questo tipo nel progetto. Trigger = email dell'Utente autenticato (nessuna nuova icona), ancorato in fondo alla barra di navigazione (`nav-bar`), stesso punto dove viveva il solo pulsante "Esci" prima di questa storia. La tendina (`role="menu"`, due voci `role="menuitem"`: "Modifica password", "Esci") **non riusa i colori chiari** di `nav-bar` (`{colors.surface-alt}` su navy) perché non condivide il suo sfondo navy: `background: {colors.surface}`, testo `{colors.text-primary}`, `{rounded.sm}` (non `{rounded.md}`, riservato a contenitori più ampi come `riquadro-form`), ombra `0 1px 3px rgba(16,24,32,0.08)` — stesso identico valore hard-coded già riusato ad hoc da `riquadro-form` (Story 9.3), coerente con l'ipotesi di lavoro della nota ora chiusa. Non un vero modale: nessun `role="dialog"`/`aria-modal`, stesso principio già scelto per il drawer di navigazione mobile (`nav-bar`, Story 9.2) — un dropdown ancorato che si chiude al click fuori o con Esc, senza bloccare l'interazione con il resto della pagina.
- **Larghezza massima di pagina (trasversale, non un componente a sé)** — estensione del 2026-07-26 (feedback utente su Story 9.3): ogni pagina dell'app, incluse quelle a contenuto tabellare/elenco (`/gruppi`, `/palestre`, `/admin`, `/slot`, `/conferma-iscrizioni`, `/conferma-certificati`, `/wizard-nuova-stagione`, ecc.), ha il proprio `<main>` centrato con `max-width: 1000px` (selettore trasversale `.contenuto > main` in `app/globals.css`, non una classe da applicare pagina per pagina) — un margine visibile anche sui laptop più comuni (13"-14", non solo sui monitor larghi), non solo un limite per gli schermi molto ampi. Le tabelle troppo larghe per 1000px continuano a scorrere orizzontalmente al proprio interno (`.scrollWrapper`, Story 8.3), non a forzare la pagina oltre il limite. **Distinto** dal riquadro-form (che aggiunge bordo/ombra/sfondo ed è più stretto, 480px): questo è solo un contenitore invisibile, applicato ovunque.
- **Larghezza massima dei campi di input (trasversale, non un componente a sé)** — estensione del 2026-07-26: ogni `<input>` di testo (non checkbox/radio), `<select>` e `<textarea>` ha `max-width: 400px` (regola globale sul tag in `app/globals.css`, non su una classe) — corregge lo stretch di default del pattern `.campo` (quasi ovunque un flex-column senza `align-items` esplicito, che allargava i singoli campi a piena larghezza del contenitore anche quando questo era molto largo, es. dentro una pagina-tabella).
...
| Badge "certificato scaduto" a livello di singola atleta in tono warning (informativo) — **eccetto** in `/conferma-certificati` (Story 9.23) e i badge Iscrizione/Tesseramento di `/gruppi` (Story 9.33, round 4), dove è danger per motivi di contesto (vedi Componenti → Badge) | Usare rosso/danger acceso a livello di singola riga fuori da quelle due eccezioni esplicite — allarma proprio nel momento in cui si registra la presenza (contro FR-15) |
