...
- Coppie di contrasto calcolate in `DESIGN.md → Colori`: testo `{colors.blu-carbone}`/`{colors.bianco}` su `{colors.azzurro}` (pulsante primario, 5.51:1), testo chiaro su `{colors.blu-carbone}` (4.76:1-11.58:1 a seconda del tono — range ricalcolato dopo il cambio da quasi-nero a blu carbone, Story 18.15, il tono più basso/`{colors.testo-scuro-muto}` ha richiesto una seconda correzione di hex per restare sopra AA sul nuovo sfondo), link "Accedi" su blu-carbone (4.97:1) — tutte sopra soglia AA dopo la correzione. I due blocchi partite (`next-match-strip`, `match-card`) hanno un secondo sfondo scuro dedicato, `{colors.azzurro-partite}` (Story 18.15, seconda iterazione, su richiesta esplicita dell'utente): testo bianco 5.46:1, testo secondario dedicato `{colors.testo-chiaro-partite}` 4.89:1, divisore "vs" `{colors.magenta-chiaro}` 3.88:1 (vincolato a testo grande). **Rimane esplicitamente da verificare** (non ancora calcolabile oggi) il contrasto del titolo hero sopra una foto reale, quando arriverà: l'ombra testo prevista nel mockup è un aiuto, non una garanzia per ogni scatto futuro — vedi `DESIGN.md → Componenti → hero`.
...
- **Ripreso dal registro "Poster Sportivo" (direzione scelta in discovery, confronto di 4 alternative):** tagli diagonali netti, tipografia condensata pesante maiuscola, blocchi di contenuto in blu carbone assertivi (header/hero/footer) e in azzurro medio dedicato (i due blocchi partite, Story 18.15 seconda iterazione), azzurro pieno come blocco colore (pulsante primario), magenta come lampo puntuale riservato.
