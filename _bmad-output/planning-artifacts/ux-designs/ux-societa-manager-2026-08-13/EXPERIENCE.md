...
- Coppie di contrasto calcolate in `DESIGN.md → Colori`: testo `{colors.nero}`/`{colors.bianco}` su `{colors.azzurro}` (6.73:1), testo chiaro su `{colors.nero}` (5.24:1-14.26:1 a seconda del tono — range corretto in accessibility review, il tono più basso/`{colors.testo-scuro-muto}` falliva AA nella prima stesura), link "Accedi" su nero (6.08:1) — tutte sopra soglia AA dopo la correzione. **Rimane esplicitamente da verificare** (non ancora calcolabile oggi) il contrasto del titolo hero sopra una foto reale, quando arriverà: l'ombra testo prevista nel mockup è un aiuto, non una garanzia per ogni scatto futuro — vedi `DESIGN.md → Componenti → hero`.
...
- **Ripreso dal registro "Poster Sportivo" (direzione scelta in discovery, confronto di 4 alternative):** tagli diagonali netti, tipografia condensata pesante maiuscola, blocchi di contenuto quasi-neri assertivi, azzurro come blocco colore pieno, magenta come lampo puntuale riservato.
