...
**Contesto tecnico verificato leggendo il codice:** `/staff` (Story 18.10, `app/staff/page.tsx`) mostra oggi solo gli Allenatori assegnati a un Gruppo nella stagione corrente (nome, cognome, foto/iniziali, elenco Gruppi) - nessun campo descrizione o ruolo aggiuntivo esiste nel modello `Allenatore` (`prisma/schema.prisma`). Nessuna pagina di gestione esiste oggi per editare dati dell'Allenatore oltre a `/app/precaricamento-allenatori` (creazione via import) - questa story introdurrebbe la prima. Mirror diretto del pattern già stabilito da Story 19.4 (foto squadra): nuova rotta **scoped** `SITE_MANAGER`-only che elenca gli Allenatori con **solo** i controlli di questa story - nessun accesso a creazione/modifica di nome/cognome/codice fiscale, nessuna assegnazione a Gruppi (resta `/app/gruppi` e `/app/precaricamento-allenatori`, invariati).

**Punti aperti, da confermare con l'utente prima dello sviluppo:**
1. **Perimetro Ruoli**: solo `SITE_MANAGER`, o anche `ADMIN`/`DIRIGENTE` (che già gestiscono altri contenuti pubblici affini)? Ipotesi di lavoro: `SITE_MANAGER` + `ADMIN` + `DIRIGENTE`, stesso principio "affianca, non sostituisce" già seguito in tutta l'Epic 19.
2. **Forma dei "ruoli aggiuntivi"**: etichette libere ripetibili (un elenco aggiungibile/rimovibile voce per voce) oppure un singolo campo di testo libero con le etichette separate da virgola? Ipotesi di lavoro: campo array (`String[]` in Prisma/Postgres, `@default([])`) - prima volta che questo pattern viene usato nel progetto (nessun precedente `String[]` in `schema.prisma`), da confermare che sia preferibile a una tabella di giunzione dedicata (scartata per ora: le etichette non hanno bisogno di essere condivise/riutilizzate tra Allenatori diversi).
3. **Lunghezza massima della descrizione**: nessun vincolo esplicito richiesto dall'utente - ipotesi di lavoro, nessun limite lato DB (mirror di `Sponsor.descrizione`, `String` libera), eventuale suggerimento visivo solo lato form.
4. **Posizione in `/staff`**: ruoli aggiuntivi e descrizione dove compaiono rispetto al nome e all'elenco Gruppi allenati? Ipotesi di lavoro: ruoli aggiuntivi subito sotto il nome (identità/ruolo prima del "cosa allena"), descrizione sotto ancora.

**Acceptance Criteria (bozza, da perfezionare in fase di create-story dopo aver risolto i punti aperti sopra):**

1. **Given** il modello `Allenatore`, **when** viene esteso con `descrizione String?` (opzionale) e `ruoliAggiuntivi String[]` (default vuoto, punto aperto #2), **then** un Allenatore esistente senza questi dati continua a funzionare invariato (nessuna regressione su `/staff`, `/squadre`, `/app/gruppi`, import allenatori)
2. **Given** un Utente con i Ruoli ammessi (punto aperto #1), **when** apre la nuova pagina scoped di gestione Staff, **then** vede l'elenco degli Allenatori (stesso filtro di `/staff`: assegnati a un Gruppo nella stagione corrente) con un form per impostare descrizione e aggiungere/rimuovere ruoli aggiuntivi per ciascuno
...
4. **Given** un Visitatore del sito pubblico, **when** apre `/staff`, **then** vede, per ogni Allenatore che ne ha almeno uno, i ruoli aggiuntivi e/o la descrizione impostati - un Allenatore senza questi dati resta visibile come oggi (nessun campo vuoto mostrato)
5. **And** un Utente senza i Ruoli ammessi (punto aperto #1) resta bloccato sulla nuova rotta, stesso pattern di ogni altra rotta protetta
