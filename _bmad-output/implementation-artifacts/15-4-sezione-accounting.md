...
1. **Given** un Admin **When** apre la navigazione **Then** vede "Accounting" come ultima voce (non più "Amministrazione"), che espansa mostra `/admin`, `/precaricamento-allenatori`, `/permessi-accesso` — **esteso in AC #4 sotto (estensione 2026-08-06)** ad includere anche `/permessi-certificati`
...
3. ~~**And** `/permessi-certificati` (voce distinta, non menzionata nella richiesta originale) resta dov'è oggi, non spostata dentro "Accounting" — non assumere che vada inclusa solo perché tematicamente simile~~ — **ribaltato dall'estensione 2026-08-06**: era un fraintendimento dell'appunto originale dell'utente, non una decisione voluta. Vedi AC #4.
4. **[Aggiunto 2026-08-06] Given** un Admin **When** apre la navigazione **Then** il gruppo "Accounting" espanso mostra anche `/permessi-certificati` come quarta figlia (dopo `/permessi-accesso`) — richiesta esplicita dell'utente, che nel suo appunto originale intendeva davvero includerla; l'esclusione dell'AC #3 nasceva da un fraintendimento in fase di analisi dell'epic, non da una decisione di prodotto deliberata.
...
- [x] Task 5 (estensione 2026-08-06): Spostare `/permessi-certificati` dentro "Accounting" (AC: #4)
  - [x] `lib/auth/route-guard.ts`: rimossa la riga standalone `/permessi-certificati` (era prima di `/dati-fisici`), aggiunta come quarta entry del blocco Accounting (dopo `/permessi-accesso`) con `gruppo: "Accounting"` — stessa posizione-conta-per-`raggruppaVociNavigazione` già documentata sopra per `/admin`.
  - [x] Commento obsoleto rimosso da `/permessi-accesso` (citava l'esclusione di `/permessi-certificati` come decisione voluta).
  - [x] `lib/auth/voci-navigazione.test.ts`: 4 test aggiornati (elenco Admin diretto, conteggio gruppo Accounting 3→4, ordine completo, figlie del gruppo) + 2 commenti "tre rotte"→"quattro rotte" per accuratezza (Allenatore/Dirigente non vedono Accounting).
  - [x] `npx vitest run`, `npx tsc --noEmit`, `npm run lint`, `npm run build` puliti — 942/942 test (nessuna variazione di conteggio: 4 test modificati, 0 aggiunti/rimossi).
...
- 2026-08-06: **Estensione post-done**, richiesta esplicita dell'utente dopo aver notato la discrepanza rileggendo un proprio appunto in `docs/appunti.txt` ("spostare permessi certificati sotto il menu accounting", mai marcato come fatto). L'AC #3 originale ("`/permessi-certificati` resta fuori, non menzionata nella richiesta") era in realtà un fraintendimento dell'analisi di apertura dell'Epic 15, non una decisione di prodotto — l'appunto originale dell'utente chiedeva davvero lo spostamento. `/permessi-certificati` spostata dentro "Accounting" come quarta figlia (dopo `/permessi-accesso`) — nuovo AC #4, AC #3 ribaltato. 4 test aggiornati in `voci-navigazione.test.ts` (nessun test aggiunto/rimosso, stesso conteggio totale). 942/942 test Vitest passati, `eslint`/`tsc --noEmit` puliti, build di produzione riuscita. Status invariato: done.
