...
- Story 9.8: Durata della sessione di login — **done**
- Story 9.9: Gestione Allenatori precaricati (vista, modifica, cancellazione) — **done**
- Story 9.10: La voce di navigazione attiva non si aggiorna durante la navigazione — **done**
- Story 9.11: Recupero password ("password dimenticata") — backlog
...
- Story 9.7: il componente di navigazione ritorna già `null` in assenza di sessione (comportamento server corretto). Dopo il `redirect()` della Server Action di logoff la barra/drawer resta comunque visibile su `/accedi`. **Causa confermata (2026-07-27)**, consultando `node_modules/next/dist/docs/`: i layout condivisi restano nella Client Cache del router Next.js e non vengono ri-richiesti durante una navigazione, nemmeno quella innescata da `redirect()` in una Server Action. Fix documentato da Next.js stesso: `revalidatePath("/", "layout")` in `esci()` prima del `redirect()`. Distinto dal gap deferito per 9.1 (bfcache sul tasto "indietro", navigazione all'indietro): questo è navigazione in avanti causata dal logoff stesso.
...
- Story 9.8: **decisione presa (2026-07-27)** — opzione (a) (impostazioni native Supabase "Time-box user sessions"/"Inactivity timeout") scartata: richiede il piano **Pro** di Supabase, verificato dall'utente dal vivo, in conflitto con NFR6. Scelta l'opzione (b): timeout applicativo per inattività, soglia **1 ora**, agganciato in `middleware.ts` (cookie `ultima-attivita`, logica pura estratta in `lib/auth/sessione-inattiva.ts`), riusando `getRouteDecision` esistente per il redirect (nessuna logica di redirect duplicata).
- Story 9.10: causa CONFERMATA (2026-07-28) — stessa causa già confermata per Story 9.7 (Client Cache del layout radice), ma qui si manifesta ad ogni navigazione normale, non solo dopo il logoff, perché `NavBar.tsx` (Server Component) calcolava la voce attiva lato server e non veniva mai ri-eseguito se il layout resta cachato. Risolto spostando il calcolo di `attiva` in `NavBarClient.tsx`, dentro il render, usando `pathname` da `usePathname()` (già presente, prima usato solo per chiudere il drawer) — si aggiorna ad ogni navigazione, incluso indietro/avanti del browser.
- Story 9.11: aggiunta il 2026-07-28 su richiesta esplicita dell'utente — funzionalità di recupero password ("password dimenticata") per un Utente che non riesce ad accedere. Verificato che oggi **non esiste alcun flusso del genere** nel progetto: `/accedi` è solo login, `/modifica-password` (Story 9.4) richiede una sessione già attiva. Da chiarire in fase di creazione storia: Supabase Auth espone `resetPasswordForEmail()` nativo (richiede un provider SMTP configurato lato Supabase per l'invio dell'email), distinto dalla configurazione SMTP applicativa propria del progetto (`configurazione_smtp`, Story 7.x/11.3, usata da `lib/email/invia-email.ts` per email custom) — non è detto siano lo stesso meccanismo, da confermare in sviluppo quale via usare.
