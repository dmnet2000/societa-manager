...
- Story 9.10: causa probabile, collegata alla stessa causa confermata per Story 9.7 (Client Cache del layout radice) — ma qui si manifesta ad ogni navigazione normale, non solo dopo il logoff, perché `NavBar.tsx` (Server Component) calcola la voce attiva lato server e non viene mai ri-eseguito se il layout resta cachato. `NavBarClient.tsx` legge già `usePathname()` lato client (oggi solo per chiudere il drawer) — possibile base per spostare lì il calcolo della voce attiva, da confermare in sviluppo.
