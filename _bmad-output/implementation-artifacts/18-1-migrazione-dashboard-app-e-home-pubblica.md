...
Story 17.1/17.2 hanno introdotto un secondo elenco (`CONTENUTI_GUIDA`) il cui campo `rotta` è un mirror manuale — non derivato — di `PROTECTED_ROUTES[].prefix`. Un test di coerenza esiste già (`lib/guida/contenuti.test.ts`, verifica `CONTENUTI_GUIDA` vs `PROTECTED_ROUTES`) e fallirà se le 29 voci non vengono aggiornate in coppia con `route-guard.ts` — ma il sintomo *in produzione* di dimenticarsene (prima che il test lo becchi) sarebbe silenzioso: nessun errore, solo l'icona "?" della guida che sparisce da ogni pagina (`contenutoPerRotta` ritorna `null` per design). Regola di processo permanente del progetto (salvata anche in memoria dell'agente): ogni story che tocca una funzionalità già documentata in guida deve aggiornarne anche il contenuto — qui si applica in modo particolarmente esteso, a tutte le 29 voci contemporaneamente.
...
Code review con 3 layer adversariali paralleli (Blind Hunter, Edge Case Hunter, Acceptance Auditor) sul diff completo `f2222a2..HEAD` (branch `feature/sito-statico`, mergiato su `main` via PR #1, commit `336402e` + merge `0180d83`).

**Acceptance Auditor**: nessuna violazione dei 8 AC — verificati singolarmente, incluse le 5 correzioni extra già annotate nelle Completion Notes.

- [x] [Review][Patch] Il testo discorsivo (`corpo`) di 3 voci di `CONTENUTI_GUIDA` menziona ancora `/orari`/`/slot` senza prefisso `/app` — lo sweep del Task 6 aveva aggiornato solo il campo `rotta` (macchina-leggibile), non le menzioni dentro il testo che un Utente legge davvero nel pannello di aiuto. Un click su quel riferimento testuale (se mai reso come link in futuro) o anche solo la lettura porta a un URL che oggi fa 404. Trovato dal Blind Hunter. [lib/guida/contenuti.ts:41,103,114]
- [x] [Review][Patch] `app/manifest.ts` imposta `start_url: "/app"` senza un campo `scope` esplicito — per l'algoritmo di default dello standard Web App Manifest, lo scope di default è la cartella di `start_url` con l'ultimo segmento rimosso, cioè `"/"` (l'intero dominio, sito pubblico incluso). L'app installata come PWA (AC #6) resta quindi navigabile fuori dai confini della dashboard una volta dentro la finestra standalone, anche se si apre correttamente su `/app`. Trovato dall'Edge Case Hunter. [app/manifest.ts]
- [x] [Review][Patch] `lib/auth/route-decision.test.ts`: il test "allows an authenticated user on a route with no role restriction" continua ad asserire `getRouteDecision("/", true, ["ATLETA"])` — prima della story verificava un Utente autenticato su una rotta protetta senza restrizione di Ruolo (`"/"` era la dashboard); ora `"/"` è pubblica, quindi il test è diventato ridondante con il test appena aggiunto sopra e non copre più nessun caso di autenticazione reale. Nessun test sostituisce la copertura sul caso equivalente per `/app`. Trovato dal Blind Hunter. [lib/auth/route-decision.test.ts:63]
- [x] [Review][Patch] `app/page.tsx` (nuova home pubblica) legge `leggiInfoLogo`/`leggiNomeSettore` in sequenza (due `try/catch` separati, uno dopo l'altro) invece che in `Promise.all` — mirror del pattern di `/accedi` (Story 8.2, mai rivisto), ma il pattern più recente e corretto per due letture indipendenti è già stabilito altrove nella stessa story (`impostazioni/page.tsx`, Story 17.2 review fix). Qui la pagina è ora esposta a qualunque visitatore anonimo di internet, non solo a Staff interno: la latenza extra la paga il pubblico. Trovato dal Blind Hunter. [app/page.tsx]
- [x] [Review][Patch] Le Dev Notes di questa stessa story (due punti) parlavano di "tutte le 31 voci" di `CONTENUTI_GUIDA` da aggiornare — l'array ne contiene esattamente 29, mai 31, in nessun punto del codice. Nessun impatto funzionale (le 29 voci reali erano corrette), solo un refuso di tracciabilità nella story. Trovato dal Blind Hunter, corretto (31 → 29 in entrambi i punti). [_bmad-output/implementation-artifacts/18-1-migrazione-dashboard-app-e-home-pubblica.md]

- [x] [Review][Defer] L'ordine "migrazione dati PRIMA o CONTESTUALMENTE al deploy del codice" (vedi Dev Notes) resta affidato a un promemoria testuale, nessun gate CI/CD lo impone (nessuna pipeline nel repo) — stesso rischio già vissuto e accettato come convenzione di processo del progetto per Story 11.1/11.2, non introdotto da questa story. Trovato dal Blind Hunter. — deferred, pre-existing (convenzione di processo già accettata nel progetto, non un gap di questa storia) [docs/deploy-produzione.md]
- [x] [Review][Defer] Un artefatto di cache Vitest (`node_modules/.vite/vitest/.../results.json`) tracciato in git dentro una cartella di rotta è sopravvissuto al `git mv` invece di essere ripulito — `.gitignore` ha solo `/node_modules` ancorato alla root, non copre sottocartelle `node_modules` innestate. Preesistente, solo trasportato meccanicamente dallo spostamento. Trovato dal Blind Hunter. — deferred, pre-existing (problema di igiene del repo indipendente da questa storia) [app/app/(onboarding-import)/import-atlete/]

**Dismessi come rumore/verificati non riproducibili (3)**: `isPublicRoute("/")` con `pathname` a doppio slash iniziale (`//app/admin`) come possibile bypass di autenticazione (Edge Case Hunter) — verificato empiricamente contro il worker di preview reale: Next.js normalizza il path con un `308 Permanent Redirect` verso la forma canonica (`Location: /app/admin`) **prima** che la logica del Proxy/middleware venga anche solo valutata, nessun contenuto protetto viene mai servito al path con doppio slash; la migrazione dati `permessi_rotte` non verifica esplicitamente collisioni con il vincolo `@@unique([rotta, ruolo])` o un valore letterale `/app` (Blind Hunter) — nessuna riga reale nei dati attuali corrisponde a questo scenario, migrazione già applicata con successo in produzione senza errori; `wrangler.preview.jsonc` non documentato nella story (Blind Hunter + Acceptance Auditor, stesso finding) — file di configurazione per un worker di preview creato durante questa sessione per testare la story dal vivo, non un deliverable della Story 18.1, nessun impatto su rotte/autenticazione applicative.
...
- 2026-08-12: Mergiata su `main` via PR #1 (commit `336402e` + merge `0180d83`). Code review completata (3 layer adversariali paralleli: Blind Hunter, Edge Case Hunter, Acceptance Auditor) - nessuna violazione degli 8 AC (Acceptance Auditor). 5 patch applicati: menzioni testuali di `/orari`/`/slot` senza prefisso dentro il `corpo` di 3 voci guida (il campo `rotta` era già corretto, mancava il testo discorsivo); `app/manifest.ts` senza `scope` esplicito (default risolveva a `"/"`, l'intero dominio, non solo `/app`); test `route-decision.test.ts` diventato ridondante dopo che `"/"` è diventata pubblica, nessuna copertura sostitutiva per l'equivalente `/app`; `app/page.tsx` con due letture indipendenti in sequenza invece che `Promise.all` (ora esposta a traffico pubblico anonimo, non solo Staff); refuso "31 voci" invece di "29" nelle Dev Notes della story stessa. 2 defer (ordine migrazione dati affidato solo a promemoria testuale senza gate CI/CD - convenzione di processo già accettata dal progetto; artefatto di cache Vitest tracciato in git, preesistente). 3 dismessi (bypass teorico di `isPublicRoute` via doppio slash iniziale - verificato empiricamente non riproducibile, Next.js normalizza con un 308 prima che il Proxy valuti l'autorizzazione; collisione teorica della migrazione `permessi_rotte` - nessun dato reale la innesca; `wrangler.preview.jsonc` non documentato - file di un worker di preview creato per testare la story dal vivo, non un deliverable). 1062/1062 test Vitest passati (invariato), 0 errori tsc/eslint, build produzione riuscita dopo i fix. Status: done.
