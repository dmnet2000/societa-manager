...
- 2026-08-10: Estensione post-review con l'aiuto di Sally (UX designer): confrontate con l'utente due opzioni di posizionamento in homepage tramite un mockup Artifact a due colonne (carosello come primo elemento vs. carosello sticky) - decisione: carosello spostato PRIMA della card di saluto in `app/page.tsx` (era dopo), non reso sticky. Motivazione: la sidebar del progetto è sticky solo da desktop in su (`@media(min-width:880px)`), un carosello sticky sarebbe stato il primo elemento fisso su mobile, con un costo permanente di spazio verticale mai più liberato al crescere della home - scartato per questo motivo verificato nel codice, non solo per gusto estetico. Nessuna modifica a `SponsorCarosello.tsx`/CSS, solo riordino JSX in `page.tsx`. 1040/1040 test Vitest passati (invariato), 0 errori tsc/eslint. Status invariato: done.
