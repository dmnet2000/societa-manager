...
- 2026-08-04: **Correzione architetturale post-done (Story 12.4)** — `leggiAbilitati()` riscritta: non più Prisma diretto (`prisma.permessoRotta.findMany`), ora Supabase REST (`createAdminClient()`, `@supabase/supabase-js`). Motivo: verificato dal vivo con `next dev` (credenziali reali) che il Proxy (`middleware.ts`, `runtime: "experimental-edge"` obbligatorio per Cloudflare) crashava su OGNI richiesta del sito una volta che questo modulo veniva raggiunto transitivamente da `route-decision.ts`/Story 12.3 — Prisma/`pg` richiede moduli Node nativi (`net`/`tls`/`util/types`) assenti nel runtime edge, non un limite teorico. La cache/TTL/fail-closed/firma pubblica di `rottaAbilitataPerRuolo`/`invalidaCachePermessi` restano invariate — cambia solo come `leggiAbilitati()` recupera i dati. Nuova migrazione `20260804010000_grant_permessi_rotte_service_role` (GRANT mancante, stesso gap già visto per `atlete`/`configurazione_smtp`/`certificati_medici`). Dettaglio completo in `12-4-poc-precaricamento-allenatori.md`. 12 test di `permessi-configurabili.test.ts` riscritti (mock di `@/lib/auth-admin/client` invece di `@/lib/prisma`), stesse asserzioni di comportamento.
