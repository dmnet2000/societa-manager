...
- 2026-07-29: Fix di layout post-done su feedback utente dal vivo — l'elenco Slot con un campo per riga era illeggibile (troppo verticale, specialmente ripetuto per ogni riga dell'elenco). Nuova classe `.campiRiga` (flex-wrap) applicata sia a `SlotRow.tsx` sia a `NuovoSlotForm.tsx` (per coerenza visiva tra form di creazione e di modifica): i 5 campi ora si dispongono in orizzontale andando a capo, tipicamente due righe a schermo pieno, un campo per riga su mobile (NFR3). Nessuna modifica alla logica, solo markup/CSS - 699/699 test invariati, 0 errori tsc/eslint. Verifica visiva dal vivo non eseguibile in questa sessione (rotta protetta, nessuna istanza Supabase locale disponibile) - da confermare con l'utente dopo il deploy.
