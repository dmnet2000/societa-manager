...
- **2026-08-21 (bug di produzione, segnalato dall'utente):** `ERR_FAILED` aprendo `/app/pagine-pubbliche` su volleymogliano.it. Causa confermata: il defer "verifica runtime `isomorphic-dompurify` sotto Cloudflare Workers non ancora fatta" (riga sopra) si è concretizzato - `isomorphic-dompurify` istanzia `new JSDOM(...)` a livello di modulo, incondizionato (`node_modules/isomorphic-dompurify/dist/index.js:49-50`), eseguito su ogni richiesta a qualunque rotta che importa `lib/sanitizza-html.ts` transitivamente (anche `/app/pagine-pubbliche`, che non chiama mai `sanitizzaHtml()` ma importa `lib/pagine-pubbliche.ts`) - jsdom funziona in Node (build locale mai lo rivelava) ma non nell'isolate V8 di Workers. **Risolto**: `lib/sanitizza-html.ts` riscritta su `sanitize-html` (htmlparser2, puro JS, nessun DOM) invece di `isomorphic-dompurify` - stessa allowlist, stesso hook `rel="noopener noreferrer"` (ora `transformTags`). Tutti gli 8 test di `lib/sanitizza-html.test.ts` verdi (1 assert adattata al formato di serializzazione `<br />` vs `<br>`, nessuna differenza funzionale). Non verificabile dal vivo in locale (`cf:preview` rotto su questa macchina) - da confermare dall'utente dopo il deploy. Dettaglio completo in `deferred-work.md`.
