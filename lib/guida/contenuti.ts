...
export const CONTENUTI_GUIDA: ContenutoGuida[] = [
  ...
  },
  {
    ...
    corpo: [
      ...
      "Dentro un'Edizione, in \"Slot orari/Palestre\" crei gli orari di gioco disponibili (etichetta libera, data, ora, Palestra, e la fase a cui sono riservati - girone, semifinale, finale vincenti o finale perdenti, con il tabellone 1°-4° o 5°-8° per queste ultime tre). Per la fase a gironi non scegli una singola Palestra: vedi invece una checklist con una riga per ciascun Campo di ciascuna Palestra già censita nel gestionale (o una riga \"sola Palestra\" per quelle senza Campi), tutte preselezionate - deseleziona quelle che non ti servono prima di creare gli Slot, utile quando più gare si giocano in contemporanea su Campi diversi della stessa Palestra o su Palestre diverse. Per semifinali e finali scegli invece la Palestra specifica, come sempre: se quella Palestra ha più Campi censiti compare anche un menu per scegliere quale, ma resta facoltativo - puoi lasciarlo su \"Nessuno\". Ovunque uno Slot ha un Campo assegnato, il suo nome compare accanto al nome della Palestra. Uno Slot già assegnato a un incontro non è cancellabile: rimuovi prima l'assegnazione. Con la matita puoi invece modificare uno Slot esistente (etichetta, data, ora, Palestra e, solo per la fase a gironi, il Campo) anche se è già assegnato a un incontro: la fase e il tabellone restano invece sempre fissi, per non spezzare l'assegnazione già fatta. Fa eccezione uno Slot di semifinale/finale con un Campo assegnato (possibile solo scegliendolo in creazione): non è modificabile affatto, nemmeno per la sola etichetta - l'icona Modifica è disattivata con una spiegazione; se non è già assegnato a un incontro puoi cancellarlo e ricrearlo con i dati corretti, altrimenti rimuovi prima l'assegnazione dallo stesso incontro.",
