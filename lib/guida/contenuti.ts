...
export const CONTENUTI_GUIDA: ContenutoGuida[] = [
  ...
  },
  {
    ...
    corpo: [
      ...
      "Qui imposti anche l'URL della Pagina Instagram della società (editabile anche da Site Manager): a differenza di Facebook, qui c'è solo un'icona cliccabile che apre la Pagina, senza alcun carosello di post - compare nel footer di ogni pagina pubblica e nella pagina \"Contatti\", affiancata a quella di Facebook se entrambe sono configurate.",
  ...
  },
  {
    ...
    // Story 20.29: accanto ai nomi, un bottone per Settimana nasconde dalla
    // pagina pubblica /torneo le Categorie di quella Settimana gia' concluse
    // (tabellone generato e classifica finale completa) - l'area
    // amministrativa continua a mostrare sempre tutte le Categorie.
    // Story 20.20: finche' il tabellone non e' generato, "Tabellone
    // semifinali/finali" mostra anche un prospetto ipotetico di sola
    // lettura degli accoppiamenti di seconda fase, con placeholder di
    // posizione al posto delle squadre reali. Story 20.21: con 4 squadre
    // per girone, ogni riga del prospetto ha un menu di prenotazione
    // anticipata dello Slot. spec-20-26: il formato a 3 squadre per girone
    // ha ora un percorso di generazione reale del tabellone (2 semifinali
    // 1°-4° + un'unica finalina diretta 5°-6°, nessuna semifinale ne'
    // finale perdenti per quel posizionamento) - la prenotazione anticipata
    // dello Slot e' quindi disponibile anche li', non solo con 4 squadre
    // per girone. spec-20-27 (review fix): lo stesso prospetto ipotetico e'
    // ora visibile anche sulla pagina pubblica /torneo (sola lettura, senza
    // il menu di prenotazione anticipata che resta solo qui in admin).
    rotta: "/app/torneo",
    titolo: "Torneo",
    ruoliAmmessi: ["ADMIN", "DIRIGENTE"],
    corpo: [
      ...
      "Accanto ai nomi delle Settimane trovi anche \"Nascondi le Categorie concluse\", un bottone per Settimana: attivalo per far sparire dalla pagina pubblica /torneo le Categorie di quella Settimana già concluse (tabellone generato e classifica finale completa), man mano che il torneo avanza - una Categoria ancora in corso resta sempre visibile, anche a bottone attivo, e una Categoria che si conclude dopo l'attivazione sparisce da sola al caricamento successivo, senza bisogno di un nuovo click. Se restano nascoste tutte le Categorie dell'Edizione, la pagina pubblica mostra un messaggio esplicito al posto dell'elenco. L'area amministrativa non è mai affetta: qui continui a vedere/gestire sempre tutte le Categorie, indipendentemente da questi due bottoni.",
      ...
      "Dentro una Categoria, apri \"Risultati e classifica\" per generare il calendario degli incontri di girone (tutti contro tutti dentro ciascun girone, generabile una sola volta e solo con almeno 2 squadre per girone) e inserire il punteggio set per set di ogni incontro (al meglio dei 3 set). Il sistema calcola da solo l'esito e i punti dell'incontro, e la classifica di ciascun girone si aggiorna subito ogni volta che modifichi un risultato. La classifica mostra anche il totale dei punti realizzati/subiti nei set (\"Punti fatti\"/\"Punti subiti\", distinti dalla colonna \"Punti\" del punteggio 3/2/1/0) ed è ordinata per punti-classifica, poi per quoziente set (set vinti/set persi), poi per quoziente punti (punti fatti/punti subiti) a ulteriore parità, infine per nome. Accanto al risultato trovi anche un campo Refertista indipendente (chi ha compilato il referto cartaceo dell'incontro, fino a 20 caratteri, facoltativo, testo libero): ha un proprio campo e un proprio bottone \"Salva\", separati da quelli del punteggio, quindi puoi assegnarlo anche prima che l'incontro abbia un risultato, e resta modificabile in ogni momento - anche su un incontro con punteggio non più modificabile perché il tabellone o le finali a valle sono già stati generati. È mostrato in sola lettura, quando valorizzato, sia qui sia sulla pagina pubblica /torneo, accanto al risultato di ogni incontro di girone, semifinale o finale.",
      "Da \"Tabellone semifinali/finali\" genera il tabellone di posizionamento (una sola volta, quando entrambi i gironi hanno la stessa classifica completa e lo stesso numero di squadre: esattamente 4, oppure esattamente 3). Con 4 squadre per girone crea le semifinali 1°A-2°B/1°B-2°A per il tabellone 1°-4° e 3°A-4°B/3°B-4°A per il 5°-8°, e le finali di entrambi i tabelloni si generano da sole non appena hai inserito il risultato delle rispettive due semifinali. Con 3 squadre per girone crea le stesse due semifinali 1°-4° (finali vincenti/perdenti automatiche come sopra), ma il 5°-6° posto è deciso subito da un'unica finalina diretta tra le due terze classificate, senza alcuna semifinale né finale perdenti per quel posizionamento. La classifica finale compare in fondo alla pagina non appena tutti gli incontri del tabellone hanno un risultato: 1°-8° con 4 squadre per girone, 1°-6° con 3.",
      "Finché il tabellone non è ancora generato, la stessa pagina \"Tabellone semifinali/finali\" mostra anche un prospetto ipotetico di sola lettura (nessuna squadra reale, nessun incontro creato) di come si incroceranno le posizioni di girone, con lo stesso schema descritto sopra per il numero di squadre in corso (4 o 3 per girone). Con qualunque altro numero di squadre per girone il prospetto non compare, e il tabellone non è generabile. Lo stesso prospetto ipotetico, sempre di sola lettura, è visibile anche sulla pagina pubblica /torneo, senza bisogno di accedere: valutane la visibilità se non vuoi che i visitatori vedano in anteprima gli accoppiamenti ipotetici prima che il girone sia concluso.",
      "Con 4 o con 3 squadre per girone, ogni riga del prospetto ipotetico (le due semifinali e le finali di ciascun tabellone, inclusa la finalina diretta 5°-6° nel formato a 3 squadre) ha anche un menu per prenotare in anticipo lo Slot su cui si giocherà, prima ancora che le squadre reali siano note: quando generi il tabellone (o quando le finali si generano da sole), quella riga riceve esattamente lo Slot prenotato, invece di ricadere sull'assegnazione automatica generica. Prenotare un nuovo Slot su una riga già prenotata sostituisce sempre la prenotazione precedente, che torna libera. Una Categoria con uno Slot ancora prenotato non è eliminabile, anche se non ha più squadre: rimuovi prima quella prenotazione.",
      ...
      "Dentro un'Edizione, in \"Slot orari/Palestre\" crei gli orari di gioco disponibili (etichetta libera, data, ora, Settimana, Palestra, e la fase a cui sono riservati - girone, semifinale, finale vincenti o finale perdenti, con il tabellone 1°-4° o 5°-8° per queste ultime tre). La Settimana è ora obbligatoria per ogni nuovo Slot: serve a far sparire dal menu di assegnazione, più sotto, gli Slot di una Settimana già conclusa e già occupati da un altro incontro. Per la fase a gironi non scegli una singola Palestra: vedi invece una checklist con una riga per ciascun Campo di ciascuna Palestra già censita nel gestionale (o una riga \"sola Palestra\" per quelle senza Campi), tutte preselezionate - deseleziona quelle che non ti servono prima di creare gli Slot, utile quando più gare si giocano in contemporanea su Campi diversi della stessa Palestra o su Palestre diverse. Per semifinali e finali scegli invece la Palestra specifica, come sempre: se quella Palestra ha più Campi censiti compare anche un menu per scegliere quale, ma resta facoltativo - puoi lasciarlo su \"Nessuno\". Ovunque uno Slot ha un Campo assegnato, il suo nome compare accanto al nome della Palestra. Uno Slot già assegnato a un incontro non è cancellabile: rimuovi prima l'assegnazione. Con la matita puoi invece modificare uno Slot esistente (etichetta, data, ora, Settimana, Palestra e, solo per la fase a gironi, il Campo) anche se è già assegnato a un incontro: la fase e il tabellone restano invece sempre fissi, per non spezzare l'assegnazione già fatta. Qui la Settimana resta facoltativa anche dopo questa storia: uno Slot creato prima dell'introduzione di questo campo non ha una Settimana finché non gliene assegni una tu esplicitamente, e nel frattempo continua a comparire in ogni menu di assegnazione senza mai essere nascosto. Fa eccezione uno Slot di semifinale/finale con un Campo assegnato (possibile solo scegliendolo in creazione): non è modificabile affatto, nemmeno per la sola etichetta - l'icona Modifica è disattivata con una spiegazione; se non è già assegnato a un incontro puoi cancellarlo e ricrearlo con i dati corretti, altrimenti rimuovi prima l'assegnazione dallo stesso incontro.",
      "Sulla pagina \"Risultati e classifica\" (girone) e \"Tabellone semifinali/finali\", ogni incontro ha un menu per assegnargli uno degli Slot compatibili con la sua fase, oppure per togliere l'assegnazione esistente. Se scegli uno Slot già assegnato a un altro incontro, l'app te lo segnala e ti chiede conferma prima di sovrascriverlo - non c'è alcun blocco, solo un avviso. Il menu nasconde automaticamente uno Slot quando, insieme, la sua Settimana è precedente a quella della Categoria che stai gestendo ED è già occupato da un altro incontro: uno Slot di Settimana passata ma ancora libero resta invece visibile, così come uno Slot occupato ma della Settimana giusta (o successiva) resta visibile con il consueto avviso \"(occupato)\". Lo Slot già assegnato a QUESTO incontro non sparisce mai dal suo stesso menu, qualunque sia la sua Settimana.",
