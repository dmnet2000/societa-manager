...
export const CONTENUTI_GUIDA: ContenutoGuida[] = [
  ...
  },
  {
    ...
    // posizione al posto delle squadre reali.
    rotta: "/app/torneo",
    titolo: "Torneo",
    ruoliAmmessi: ["ADMIN", "DIRIGENTE"],
    corpo: [
      ...
      "Finché il tabellone non è ancora generato, la stessa pagina \"Tabellone semifinali/finali\" mostra anche un prospetto ipotetico di sola lettura (nessuna squadra reale, nessun incontro creato) di come si incroceranno le posizioni di girone: con almeno 4 squadre per girone lo schema è quello descritto sopra (semifinali e finali 1°-4° e 5°-8°); con esattamente 3 squadre per girone, il 5°-6° posto è invece deciso da un'unica finalina diretta tra le due terze classificate, senza semifinali. Con qualunque altro numero di squadre per girone il prospetto non compare.",
      "Con esattamente 4 squadre per girone, ogni riga del prospetto ipotetico (le due semifinali e le due finali di ciascun tabellone) ha anche un menu per prenotare in anticipo lo Slot su cui si giocherà, prima ancora che le squadre reali siano note: quando generi il tabellone (o quando le finali si generano da sole), quella riga riceve esattamente lo Slot prenotato, invece di ricadere sull'assegnazione automatica generica. Prenotare un nuovo Slot su una riga già prenotata sostituisce sempre la prenotazione precedente, che torna libera. Con 3 squadre per girone questo menu non compare su nessuna riga, perché quel formato non genera ancora un tabellone reale. Una Categoria con uno Slot ancora prenotato non è eliminabile, anche se non ha più squadre: rimuovi prima quella prenotazione.",
