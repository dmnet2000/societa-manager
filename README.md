...
26/32 storie completate su 7 Epic. Epic 1 (Accesso, Popolamento e Iscrizioni), Epic 2 (Palestre, Gruppi e Orari) ed Epic 3 (Presenze, incluso il trend/percentuale) completati; Epic 4 (Compliance Visite Mediche) in corso (5/6: upload certificato, notifica automatica, mail alla Segreteria, conferma/validazione certificato e alert scadenza non bloccante completati — resta solo la Story 4.6, promemoria scadenza). Epic 7 (Configurazione Applicazione) aggiunto in corso d'opera il 2026-07-18 e completato — la polisportiva invia le email transazionali tramite la propria casella SMTP (Aruba) invece di un provider terzo, con parametri configurabili da un'interfaccia Admin (vedi [`_bmad-output/planning-artifacts/sprint-change-proposal-2026-07-18.md`](_bmad-output/planning-artifacts/sprint-change-proposal-2026-07-18.md)) — eseguito prima di Story 4.3, da cui è un prerequisito. Stato dettagliato in [`_bmad-output/implementation-artifacts/sprint-status.yaml`](_bmad-output/implementation-artifacts/sprint-status.yaml), vista sintetica in [`docs/stato-avanzamento-progetto.html`](docs/stato-avanzamento-progetto.html), roadmap discorsiva in [`docs/panoramica-prodotto.md`](docs/panoramica-prodotto.md).
...
- Recupero password (Story 9.11): il link di reset è generato con `supabase.auth.admin.generateLink({ type: "recovery" })` (service-role, `lib/auth-admin/client.ts`) e inviato tramite l'SMTP applicativo esistente (`lib/email/invia-email.ts`), **non** tramite le impostazioni email native di Supabase — coerente con AD-12 (configurazione email gestita dall'Admin via DB, non da un servizio esterno separato). La pagina `/reimposta-password` verifica il token con `supabase.auth.verifyOtp({ token_hash, type: "recovery" })`. L'Admin ha inoltre un reset diretto a password fissa (`/admin`, Server Action `reimpostaPasswordFissaUtente`) per sbloccare un Utente senza passare dall'email.
